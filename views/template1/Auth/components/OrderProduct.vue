<template>
    <div
        v-for="(product, index) in order" :key="index"
    ...
    >
            ...
            <div class="flex flex-col gap-3">
                <template v-for="(column, columnIndex) in columns" :key="columnIndex">
                    <div class="grid grid-cols-2 gap-4 text-gray-700" v-if="product?.[column.prop]">
                        ...
                        <div class="flex flex-col gap-1" v-if="column.array === true">
                ...
                <div v-if="product.rule">
                    <div class="inline-block text-gray-500 text-sm px-3 py-1 border-[1px] border-gray-500"
        ...
        <div v-if="isDoor" class="flex gap-2 items-center h-fit cursor-pointer">
            ...
            />
            <div class="underline underline-offset-2" @click="handleDetail">查看商品詳情</div>
        </div>
        <client-only>
            <ProductDialog
                v-model="dialogDetail" :dialogData="dialogData"
            />
        </client-only>
    </div>
</template>
