 ...
 */
function init() {
    if (history.state.email || !user.value.is_verified) {
        userData.value.verification = true;
        userData.value.email = history.state.email ?? user.value.email;
        if (history.state.state === "會員尚未完成驗證" || !user.value.is_verified) {
